...
// dropdown wilayah
document.addEventListener('DOMContentLoaded', function () {
    const input = document.getElementById('wilayah');  // Mengubah 'wialayah' menjadi 'wilayah'
    const content = document.getElementById('wilayah-konten');
    
    input.addEventListener('click', function () {
        content.classList.toggle('hidden');
    });
    
    const kontenitem = document.querySelectorAll('.wilayah-item');
    kontenitem.forEach(item => {
        item.addEventListener('click', function () {
            input.value = item.querySelector('.value').textContent;
            input.dataset.value = item.dataset.value;
